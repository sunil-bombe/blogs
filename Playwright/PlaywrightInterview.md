...
A comprehensive guide for beginners to advanced engineers.
...
# Additional Advanced Playwright Interview Questions

## 1. What is `test.use()`?

Used to override configuration such as browser, locale, storageState,
viewport, permissions, or baseURL for a specific test or suite.

## 2. What is `storageState`?

It stores authentication cookies and local storage so you can reuse
logged-in sessions without logging in every test.

## 3. Difference between `browser.newPage()` and `context.newPage()`

-   `browser.newPage()` creates a new temporary context.
-   `context.newPage()` uses an existing context, allowing shared
    cookies and session.

## 4. How do you intercept and modify network requests?

Use `page.route()` and `route.continue()` to modify headers, payloads,
or URLs.

## 5. How do you run Playwright tests in Docker?

Use the official Playwright Docker image and execute
`npx playwright test` inside the container.

## 6. What is `expect.poll()`?

Retries a custom assertion until it succeeds or times out.

## 7. How do you authenticate once for all tests?

Use a global setup to generate `storageState` and reuse it.

## 8. How do you handle geolocation testing?

Grant geolocation permission and set latitude/longitude in the browser
context.

## 9. How do you test browser permissions?

Configure permissions such as `clipboard-read`, `notifications`, or
`geolocation` using the browser context.

## 10. What Playwright features help reduce flaky tests?
